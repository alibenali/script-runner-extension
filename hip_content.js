...
async function delay() {
  ...
  return new Promise((resolve) => {
      chrome.storage.sync.get('hipDelay', function(data) {
          const delayTime = (data.hipDelay || 5) * 1000; // Default to 5 seconds if not set
...
async function phys1(){
  ...
  //'div[aria-label*="Same as Active ROM"]' Fifth one
  await waitForElement('div[aria-label*="Same as Active ROM"]', number=4).then((element) => {
    element.querySelectorAll('input[type="checkbox"]')[0].click();
  });
  
  //'div[aria-label*="Same as Active ROM"]' Sixth one
  await waitForElement('div[aria-label*="Same as Active ROM"]', number=5).then((element) => {
    element.querySelectorAll('input[type="checkbox"]')[0].click();
  });
  ...
  //'div[aria-label*="Does a limitation in passive adduction prevent the Veteran from crossing his/her legs?"]'
  await waitForElement('div[aria-label*="Does a limitation in passive adduction prevent the Veteran from crossing his/her legs?"]').then((element) => {
    element.querySelectorAll('input[type="radio"]')[1].click();
  });
...
async function additional(){
  ...
  js_path = 'div[aria-label*="Right side"]';
  await waitForElement(js_path).then((element) => {
    element.querySelectorAll('input[type="radio"]')[1].click();
  });
  
  js_path = 'div[aria-label*="Left side"]';
  await waitForElement(js_path).then((element) => {
    element.querySelectorAll('input[type="radio"]')[1].click();
  });
  ...
  js_path = 'div[aria-label*="9a"]';
  await waitForElement(js_path).then((element) => {
    element.querySelectorAll('input[type="radio"]')[1].click();
  });
  
  js_path = 'div[aria-label*="10a"]';
  await waitForElement(js_path).then((element) => {
    element.querySelectorAll('input[type="radio"]')[1].click();
  });
}
