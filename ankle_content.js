...
async function delay() {
  ...
  return new Promise((resolve) => {
      chrome.storage.sync.get('ankleDelay', function(data) {
          const delayTime = (data.ankleDelay || 5) * 1000; // Default to 5 seconds if not set
...
async function history() {
  ...
  //'div[aria-label*="Does the Veteran report that flare-ups impact the function of the "]'
  document.querySelector(
    'div[aria-label*="Does the Veteran report that flare-ups impact the function of the"]'
  ).querySelectorAll('input[type=radio]')[1].click();
...
async function phys3(){
  js_path = 'div[aria-label*="Is there absence of firm end point with asymmetric or excessive motion?"]'
  await waitForElement(js_path).then((element) => {
    element.querySelectorAll('input[type="radio"]')[1].click();
  });
  
  js_path = 'div[aria-label*="Is there asymmetric or excessive motion?"]';
  await waitForElement(js_path).then((element) => {
    element.querySelectorAll('input[type="radio"]')[1].click();
  });
  
  js_path = 'div[aria-label*="Is there absence of firm end point with asymmetric or excessive motion?"]'
  await waitForElement(js_path, number=1).then((element) => {
    element.querySelectorAll('input[type="radio"]')[1].click();
  });
